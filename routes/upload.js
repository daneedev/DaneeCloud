...
router.post("/", upload.single('file'), checkAuth, checkVerify, async function (req, res) {
    ...
    if (removeaccents.has(name)) {
      ...
      if (fs.readdirSync(__dirname + "/.." + config.uploadsfolder + `${req.user.username}/`).includes(name)) {
        res.render(__dirname + "/../views/message2.ejs", {message: `<span class="material-icons">file_copy</span>&nbsp;File ${name} already exist!`,  cloudname: config.cloudname, lang: lang})
      } else {
        ...
        if (req.file.size / (1024 * 1024).toFixed(2) + user.usedStorage > role.maxStorage) {
          res.render(__dirname + "/../views/message2.ejs", {message: `<span class="material-icons">storage</span>&nbsp;You have reached your storage limit! Try delete some files.`,  cloudname: config.cloudname, lang: lang})
        } else {
        fs.writeFile(__dirname + "/.." + config.uploadsfolder + `${req.user.username}/` +  name, req.file.buffer, async  err => {
          if (err) {
            res.send(err);
          } else {
            res.render(__dirname + "/../views/message2.ejs", {message: `<span class="material-icons">cloud_done</span>&nbsp;File ${name} uploaded succesfully!`,  cloudname: config.cloudname, lang: lang})
